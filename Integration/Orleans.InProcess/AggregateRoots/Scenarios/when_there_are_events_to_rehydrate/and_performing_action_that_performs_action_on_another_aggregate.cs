...
using context = Cratis.Chronicle.Integration.Orleans.InProcess.AggregateRoots.Scenarios.when_there_are_events_to_rehydrate.and_performing_action_that_performs_action_on_another_aggregate.context;
...
public class and_performing_action_that_performs_action_on_another_aggregate(context context) : Given<context>(context)
{
    public class context(GlobalFixture globalFixture) : given.context_for_aggregates_with_single_unit_of_work(globalFixture)
    {
        ...
        async Task Because()
        {
            await PerformInUnitOfWork(User.DoSomething);
