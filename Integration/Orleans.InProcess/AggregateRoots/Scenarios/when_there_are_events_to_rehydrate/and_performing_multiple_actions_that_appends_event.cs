...
using context = Cratis.Chronicle.Integration.Orleans.InProcess.AggregateRoots.Scenarios.when_there_are_events_to_rehydrate.and_performing_multiple_actions_that_appends_event.context;
...
public class and_performing_multiple_actions_that_appends_event(context context) : Given<context>(context)
{
    public class context(GlobalFixture globalFixture) : given.context_for_aggregates_with_single_unit_of_work(globalFixture)
    {
        ...
        async Task Because()
        {
            await PerformInUnitOfWork(async () => await User.Onboard("Some user name"));
            await PerformInUnitOfWork(async () => await User.ChangeUserName(UserName));
    ...
    [Fact]
    void should_commit_two_events() => Context.UnitOfWork.GetEvents().ToList().Count.ShouldEqual(2);

    [Fact]
    void should_not_be_deleted() => Context.ResultState.Deleted.ShouldEqual(new(false, 0));

    [Fact]
    void should_have_assigned_username_to_internal_state() => Context.ResultState.Name.ShouldEqual(new(Context.UserName, 2));
}
